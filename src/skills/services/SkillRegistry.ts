...
async function parseSkill(args: {
  ...
  const scriptPaths = await listSkillFiles(skillFullPath, 'scripts');
  const referencePaths = await listSkillFiles(skillFullPath, 'references');
  const assetPaths = await listSkillFiles(skillFullPath, 'assets');
